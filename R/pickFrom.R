                         ...
                         windowPos = "+150+30")
{
    ...
    if (graphics & capabilities("tcltk")) {
        ...
        items.list <- as.character(tclVar(paste("{", paste(vec.to.pickfrom,
                                                           collapse = "} {"),
                                                "}", sep = "")))
        ...
        for (i in 1:nsets) {
            ...
            setlabeltext[[i]] <- tklabel(labelbox[[i]], text =
        ...
        add.cmd <- deparse(function() {
            ...
            if (as.logical(tclObj(preserve.order)))
        ...
        tkbind(items, "<<ListboxSelect>>", function() {
            items.selected <- vec.to.pickfrom[1 + string.to.vector(tclvalue(tkcurselection(items)))]
            for (i in 1:nsets) {
